...
cdm$allvac_cohort <- cdm$allvac_cohort |>
  group_by(subject_id) |>
...
vac_attrition <- attrition(cdm$vac_cohort)
unvac_attrition <- attrition(cdm$unvac_cohort)
matched_vac_attrition <- attrition(cdm$total_vac_matched_cohort)
matched_unvac_attrition <- attrition(cdm$total_unvac_matched_cohort)

write.csv(matched_vac_attrition, paste0(resultsFolder,"/matched_vac_attrition_",cdmSchema,".csv"), row.names = FALSE)
write.csv(matched_unvac_attrition, paste0(resultsFolder,"/matched_unvac_attrition_",cdmSchema,".csv"), row.names = FALSE)
