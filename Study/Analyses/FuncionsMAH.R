...
addRegion <- function(x) {
  ...
  if (dbName == "CPRD GOLD") {
    ...
    cli::cli_abort("cdmName is not valid")
  }
}
...
addPreviousVaccinations <- function(x, cohort_id = NULL, index_date_name = "index_date"){
  if(is.null(cohort_id)){
    x |> PatientProfiles::addCohortIntersectCount(
      targetCohortTable = "vaccinations",
      targetCohortId    = NULL,
      indexDate         = index_date_name,
      censorDate = NULL,
      targetStartDate = "cohort_start_date",
      targetEndDate   = "cohort_end_date",
      window    = list(c(-Inf,-1)),
    ...
    if("previous_vaccines" %in% colnames(x)){
      x |> 
